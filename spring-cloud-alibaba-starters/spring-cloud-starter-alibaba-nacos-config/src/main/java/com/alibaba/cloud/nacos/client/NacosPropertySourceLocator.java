...
public class NacosPropertySourceLocator implements PropertySourceLocator {
	...
	private void loadNacosConfiguration(final CompositePropertySource composite,
		...
		for (NacosConfigProperties.Config config : configs) {
			loadNacosDataIfPresent(composite, config.getDataId(), config.getGroup(),
					NacosDataParserHandler.getInstance().getFileExtension(config.getDataId()), config.isRefresh());
		}
	}
		...
		for (int i = 0; i < configs.size(); i++) {
			...
			if (dataId == null || dataId.trim().length() == 0) {
				throw new IllegalStateException(
						String.format("the [ spring.cloud.nacos.config.%s[%s] ] must give a dataId", tips, i));
			}
		}
	}
	...
	private NacosPropertySource loadNacosPropertySource(final String dataId, final String group, String fileExtension,
