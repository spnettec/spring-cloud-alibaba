...
public class DubboCloudRegistry extends FailbackRegistry {
	...
	public DubboCloudRegistry(URL url, DiscoveryClient discoveryClient, DubboServiceMetadataRepository repository,
			...
			DubboGenericServiceFactory dubboGenericServiceFactory, ConfigurableApplicationContext applicationContext) {
	...
	private void subscribeURLs(URL subscribedURL, List<URL> subscribedURLs, String serviceName,
	...
	private void subscribeURLs(URL subscribedURL, List<URL> subscribedURLs, String serviceName,
		...
		List<URL> exportedURLs = getExportedURLs(subscribedURL, serviceName, serviceInstances);
	...
	private List<ServiceInstance> getServiceInstances(Iterable<String> serviceNames) {
		return stream(serviceNames.spliterator(), false).map(this::getServiceInstances).flatMap(Collection::stream)
